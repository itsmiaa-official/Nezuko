...
sendGroupV4Invite: {
    ...
    async value(jid, text = '', footer = '', buffer, buttons, copy, urls, quoted, options) {
        ...
        return await sendCompatibleMessage(conn, jid, {
            text,
            footer,
            buttons: buttons?.map(btn => ({
                buttonId: btn[1],
            ...
            image: buffer ? { url: buffer } : undefined
        }, { quoted, ...options })
    }
}, 
    ...
    async value(jid, title, text, buttonText, listSections, quoted, options = {}) {
        ...
        return await sendCompatibleMessage(conn, jid, {
            ...
            sections: listSections?.map(section => ({
                title: section.title || section[0],
                rows: section.rows || section[1]?.map(row => ({
                    title: row.title || row[0],
                    description: row.description || row[2] || '',
                    rowId: row.rowId || row[1]
                }))
            }))
        }, { quoted, ...options })
    }
        ...
        getName: {
            ...
            value(jid = '', withoutContact = false) {
                ...
                if (jid.endsWith('@g.us')) return new Promise(async (resolve) => {
                    ...
                    (conn.chats[jid] || {})
                let userName = null
                if (global.getUser) userName = (global.getUser(jid)?.name || global.getUser(jid.replace('@s.whatsapp.net', ''))?.name)
                if (!userName) {
                    ...
                    if (interactiveResponse) {
                        try {
                            ...
                            if (params.id) {
                                ...
                                if (!global.buttonLoggedIds) global.buttonLoggedIds = new Set()
                                if (!global.buttonLoggedIds.has(params.id)) {
                                    global.buttonLoggedIds.add(params.id)
...
/**
 * Helper function to send messages with channel button (replaces rcanal)
 ...
 */
export async function replyWithChannel(conn, chat, text, quoted = null, options = {}) {
  try {
    const buttons = []
    ...
    conn.reply(chat, `${text}\n\n🎵 *Canal Oficial:* https://www.whatsapp.com/channel/0029VajYamSIHphMAl3ABi1o`, quoted, options);
  }
}
